 ...
 */
class Editor extends GameObject
{
	private game: AGGame;
	private mouse: Mouse;
	...
	private selectedObject: GraphicalObject;
		...
		*/
		if(event.detail.down) {
			for(var i = keys.length - 1; i >= 0; i--) {
				let object: GraphicalObject = 
					<GraphicalObject> this.game.getObject(keys[i]);
	
				if(inBox(coords.x, coords.y, object)) {					
					this.selectedObject = <GraphicalObject> object;
		...
		*/
		...
		if(this.currentSprite in this.types) {
			if(event.detail.down && event.detail.buttons & 1) {
				...
				let object = constructors[type]({ x: coords.x, y: coords.y, sprite: this.currentSprite});
			...
			*/
			...
			let level = <AGLevel> this.game.getObject("level");
	
			if(event.detail.buttons & 1)
				level.setSprite(coords, this.currentSprite);
			else if(event.detail.buttons & 2)
				level.setSprite(coords, 0);
		}
	}
